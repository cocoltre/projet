...
bool Collider::isColliderInside (const Collider& other) const { // détermine si un Collider est à l'intérieur d'un autre
...
bool Collider::operator| (const Collider& body2) const { // détermine si deux Colliders sont en collision
    return (isColliding(body2));
}

bool Collider::operator> (const Vec2d& point) const { // détermine si un point est à l'intérieur d'un Collider
